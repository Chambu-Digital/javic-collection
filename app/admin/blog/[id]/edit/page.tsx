...
export default function EditBlogPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchBlogPost = async (id: string) => {
    try {
      ...
      if (response.ok) {
        const post: IBlogPost = await response.json()
        setFormData({
          title: post.title,
          ...
          relatedProducts: post.relatedProducts?.map((p: any) => typeof p === 'string' ? p : p._id) || [],
  ...
  const handleSubmit = async (e: React.FormEvent, publish?: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to update blog post: ${error.error}`)
      }
    } catch (error) {
